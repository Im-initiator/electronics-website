...
public interface TokenRepository extends JpaRepository<TokenEntity,Long> {
    ...
    @Modifying
    @Query("delete from TokenEntity t2 where t2.id IN (select tid from (select t.id AS tid from TokenEntity t where t.account.id = :userId order by t.createDate limit 1))")
