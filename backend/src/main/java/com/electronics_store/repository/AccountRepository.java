...
public interface AccountRepository extends JpaRepository<AccountEntity, Long> {
    ...
    @Query(
            value =
                    "SELECT a FROM AccountEntity a left join fetch a.roles WHERE a.state = :state and a.status = :status order by a.createDate desc",
    ...
    @Query(
            value =
                    "SELECT a FROM AccountEntity a left join fetch a.roles WHERE a.state = :state and a.status = :status and LOWER(a.userName) LIKE LOWER(CONCAT('%',:name,'%')) order by a.createDate desc",
            countQuery =
